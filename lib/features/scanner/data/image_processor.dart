...
// Phase XVI.37 — `estimateDeskewDegrees` and its `_resizeForDeskew`
// helper moved to `lib/engine/geometry/auto_straighten.dart` so the
// editor's Auto-straighten button can share the same OpenCV-backed
// estimator. The scanner-side `processOcrAware` callsite imports the
// new location directly; this file re-exports for any external
// callers that still expect the legacy import path.
//
// (No-op block intentionally left here as a docstring breadcrumb —
// the next contributor finds the new location without grepping.)
