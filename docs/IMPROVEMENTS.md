...
- ~~**Theme hydration runs async after first frame.** Moving `ThemeModeController._hydrate` into `main()` alongside `hydratePersistedLogLevel` removes the one-frame flash of dark on light-mode users. [40]~~ ✅ *Phase XI.C.4: `hydratePersistedThemeMode()` reads `SharedPreferences` in `main()` before `runApp`; `ProviderScope` override on `themeModeControllerProvider` seeds the controller with the resolved mode. Controller keeps its in-constructor `_hydrate()` as a belt-and-suspenders. First frame renders with the saved theme. 7 new tests in `theme_mode_controller_test.dart`.*
...
- ~~**`DirtyTracker._mapEquals` is shallow.** `==` on `List` / `Map` values always diverges; HSL and split-toning force unnecessary re-renders. [02]~~ ✅ *Phase XI.A.2: `_deepEquals` recurses into `List` / `Map`; HSL (8-element float lists), split-toning (`[r,g,b]` + balance), and tone-curve (nested `List<List<double>>`) rebuilds with identical values no longer force a false dirty. 4 new tests in `dirty_tracker_test.dart`.*
...
- ~~**`ShaderRenderer.shouldRepaint` always returns true.** Repaints on every ancestor `markNeedsPaint` bubble, even when no uniforms changed. Content-hash would save repaints on scroll. [03]~~ ✅ *Phase XI.A.3: `ShaderPass.contentHash` snapshots uniform values at `toPass()` build time; `shouldRepaint` compares hashes + sampler identities per pass and returns `false` when every pair matches. `ColorGradingShader` explicitly captures the reused `matrixScratch` buffer's contents at build time (frame N+1 overwrites the same buffer, so deferring would compare current frame against itself). 17 shader wrappers opted in; unopted passes fall back to the pre-XI.A.3 always-repaint path. 10 new tests in `shader_renderer_repaint_test.dart`.*
...
- ~~**OCR `runOcrIfMissing` runs pages serially.** `Future.wait` halves wall time on multi-page sessions. [32]~~ ✅ *Phase XI.C.3: new `runOcrBatch` helper drives `runBoundedParallel` with `kOcrConcurrency = 4`; the notifier pre-filters to pending pages and commits each result on the main isolate as workers finish. 5 pages × 100 ms verified at < 250 ms vs 500 ms serial. 7 new tests in `ocr_parallel_test.dart`.*
