...
- **`SaveToFilesPlugin.swift` missing from iOS target.** Phase VIII.D committed `ios/Runner/SaveToFilesPlugin.swift` but never wired it into `ios/Runner.xcodeproj/project.pbxproj`. `AppDelegate.swift` references the class → `flutter build ios` failed with "Cannot find 'SaveToFilesPlugin' in scope". At runtime `registrar(forPlugin:)` returned nil, so the method channel was never registered and Save-to-Files was a silent no-op on iOS. Fixed by adding the file to the Runner target (PBXBuildFile + PBXFileReference + group + PBXSourcesBuildPhase). Would not have surfaced without the Phase XI audit running a real iOS build. [32]
- **Editor `RenderFlex overflowed 585 px` on phone portrait.** Narrow/portrait branch of `_EditorBody` used `Column(Expanded canvas, panels)` with no scroll fallback — on iPhone 15 Pro the Color tab's stacked Lightroom + HSL + Split-Toning panels demanded ~1400 px in an 817 px viewport. Replaced with a `DraggableScrollableSheet` over the canvas, three snap points (14% / 45% / 90%). `ToolDock` gained a `scrollable` flag so the sheet's own scroll controller drives everything — no nested-scroll fight with the dock's internal `SingleChildScrollView`. [01]
- **B&W presets (Mono, Noir, Sharp B&W, B&W Gold, Silver, Platinum) produced green-tinted images.** `ColorGradingShader._setUniforms` uploaded the 4×4 color matrix row-major, but GLSL `std140` expects column-major — every color-grading pass silently transposed the matrix on the GPU. Symmetric matrices (brightness, contrast, exposure) looked identical after transpose so the bug was invisible; asymmetric matrices (saturation, hue, channel-mixer) got corrupted. At `saturation = -1.0` the Rec.709 luma weights ended up on the input-G channel instead of the output R/G/B rows, so the G coefficient (0.7152, the largest) dominated every pixel. Fixed by swapping the pack loop to column-major; extracted `packUniformBlock` as a `@visibleForTesting` pure helper (ui.FragmentShader is a `base` class outside its library so it can't be mocked). Would not have surfaced without a hands-on QA pass. 5 new tests. [03]
