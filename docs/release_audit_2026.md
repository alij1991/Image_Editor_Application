...
| A6 | **Licenses screen + NOTICES** — 🟡 IN PROGRESS (XVI.129) | `showLicensePage` already wired in Settings; it auto-lists Dart packages (incl. onnxruntime/litert/opencv_dart) but NOT the raw model-weight assets. Registering the bundled + downloadable model attributions via `LicenseRegistry.addLicense` so each shipped model + its license (Apache/MIT/BSD-3 + Apple ml-cvnets notice for MobileViT) appears. |
