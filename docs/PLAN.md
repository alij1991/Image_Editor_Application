...
- ✅ **XI.0.3 — Editor portrait layout overflowed by 585 px** (iPhone 15 Pro, Color tab). `_EditorBody`'s narrow/portrait branch was `Column(Expanded canvas, panels)` with no scroll fallback; the Color tab's stacked `LightroomPanel + HslPanel + SplitToningPanel` demanded ~1400 px in an 817 px viewport. Replaced with a `DraggableScrollableSheet` over the canvas (snap points 14 % / 45 % / 90 %). `ToolDock.scrollable` (default true) is forwarded from `_PanelStack` — the bottom-sheet host sets it to false so the sheet's own `ScrollController` drives the whole content as one scroll region.
- ✅ **XI.0.4 — B&W presets produced green images**. `ColorGradingShader._setUniforms` uploaded its 4×4 color matrix row-major, but GLSL `std140` expects column-major — every pass silently transposed the matrix on the GPU. Symmetric matrices (brightness, contrast, exposure) looked unchanged after transpose; asymmetric ones (saturation, hue, channel-mixer) broke. Saturation at `-1.0` (every B&W preset) re-routed the Rec.709 luma weights from output rows to input columns, so G (0.7152) dominated every pixel → green cast. Fixed by flipping the pack loop; extracted `packUniformBlock` as `@visibleForTesting` + 5 regression tests in `color_grading_matrix_upload_test.dart`.
